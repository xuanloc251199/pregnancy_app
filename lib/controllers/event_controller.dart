...
class EventController extends GetxController {
  ...
  //   isLoading(true);
  //   try {
  //     final fetchedUpcomingEvents = await eventRepository.fetchUpcomingEvents();
  //     events.assignAll(
  //         fetchedUpcomingEvents); // Gán danh sách sự kiện vào `events`
  //   } catch (e) {
  //     printError(info: 'Error event: Failed to fetch events: $e');
  //   } finally {
  //     isLoading(false);
  ...
  //           await eventRepository.fetchRecommendations(token);
  //       recommendEvents.assignAll(fetchedRecommendations);
  //     }
  //   } catch (e) {
  ...
  //   Duration? startCheckInTime = event.getStartTimeAsDuration();

  //   final startCheckInPeriod = startCheckInTime! - const Duration(minutes: 15);
  //   final endCheckInPeriod = startCheckInTime + const Duration(minutes: 15);
  ...
  //     if (currentTime >= startCheckInPeriod &&
  //         currentTime <= endCheckInPeriod) {
  //       printInfo(info: 'Check-in period active. Returning "check_in".');
  //       return 'check_in';
  //     }

  //     if (currentTime > endCheckInPeriod) {
  ...
  //       if (response.statusCode == 200) {
  //         print('Registration successful: ${response.data}');
  //         // Cập nhật trạng thái nút ngay sau khi đăng ký thành công
  ...
  // Future<void> checkInEvent(String eventId) async {
  //   isLoading(true);
  //   try {
  //     final prefs = await SharedPreferences.getInstance();
  //     final token = prefs.getString('token') ?? prefs.getString('temp_token');
  ...
  //       if (response.statusCode == 200) {
  //         print('Check-in successful: ${response.data}');
  //         // Cập nhật trạng thái sự kiện sau khi check-in thành công
  //         await fetchEventById(eventId);
  //       }
  //     } else {
  //       Get.log('Error: No token found.');
